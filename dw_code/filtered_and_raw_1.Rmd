...
```{r qc_raw_before, echo=TRUE, warning=FALSE, error=FALSE, message=FALSE, fig.height=5, fig.width=12}

# Cell count (for reporting)
Start_raw <- ncol(raw_1)

# QC violin plots (raw data)
VlnPlot(
  raw_1,
...
# Note: QC of the raw dataset revealed that the majority of barcodes had near-zero gene and UMI counts, consistent with empty droplets, motivating downstream filtering. No downstream clustering was performed on the raw dataset, as it primarily serves to demonstrate the necessity of filtering empty droplets.
```


# Quality control on filtered data
